...
export default function ApprovalSidebar({ requests, classCode }: ApprovalSidebarProps) {
  const [loading, setLoading] = useState(false);
  const router = useRouter();

  const handleAction = async (action: 'approve' | 'reject', requestId: number, studentId: string) => {
    setLoading(true);
    try {
      let result;
      if (action === 'approve') {
  ...
  return (
    <div className="bg-white p-4 border-l  shadow-sm h-full">
      ...
      {requests.length > 0 ? (
            ...
            {requests.map(req => (
              <div key={req.id} className="p-2 hover:bg-gray-50 rounded-lg">
                <div className="flex items-center gap-3 mb-2">
                  <Image
                    src={req.student.img || "/avatar.png"}
                    alt={req.student.username}
                    width={40}
                    height={40}
                    className="w-10 h-10 rounded-full object-cover"
                  />
                  <span className="font-medium text-gray-900">{req.student.username}</span>
                </div>
        ...
        <p className="text-sm text-gray-500 text-center py-10">Không có yêu cầu nào đang chờ.</p>
      )}
    </div>
  );
}
