...
import { CourseWithDetails, FolderWithCourseCount } from "@/app/(page)/teacher/class/[id]/video/page"; // Cập nhật đường dẫn import
...
export default function VideoList({
    ...
    const renderRow = (course: CourseWithDetails) => (
        <tr key={course.id} className="border-b border-gray-200 hover:bg-slate-50">
            {/* Tên bài giảng */}
            <td className="p-4">
                <Link href={`/${role}/class/${classCode}/video/${course.id}`} className="flex items-center gap-4 group">
                     ...
                     {openMenuId === course.id && (
                         <div className="absolute right-0 top-full mt-2 w-48 bg-white rounded-md shadow-lg border border-slate-200 z-10">
                             ...
                             <div className="px-1 py-2">
                                 <Link href={`/${role}/class/${classCode}/video/${course.id}`} className=" px-2 py-2 flex items-center gap-3 w-full text-left  text-sm text-slate-700 hover:bg-slate-100">
                                     <Eye className="w-4 h-4" /><span>Xem chi tiết</span>
                                 </Link>
                                 <Link href={`/${role}/class/${classCode}/video/${course.id}/edit`} className="px-2 py-2 flex items-center gap-3 w-full text-left  text-sm text-slate-700 hover:bg-slate-100">
        ...
        <div className="flex bg-white font-sans h-full">
            ...
            <aside className="w-64 bg-slate-50 border-r border-slate-200 p-4 shrink-0">
                <nav className="space-y-1">
                    <Link href={`/${role}/class/${classCode}/video`}
                    ...
                    {folders.map((folder) => (
                        <div key={folder.id} className="relative group flex items-center justify-between w-full">
                            <Link href={`/${role}/class/${classCode}/video?folderId=${folder.id}`} 
                     ...
                     <div className="flex items-center gap-4 w-full md:w-auto">
                         ...
                         {role === "teacher" && (
                             <>
                                 ...
                                 <Link href={`/${role}/class/${classCode}/video/add`} className="px-3 py-2 text-sm font-medium text-white bg-blue-600 rounded-lg hover:bg-blue-700 whitespace-nowrap">
                                     Tạo khóa học
                                 </Link>
                             </>
                         )}
